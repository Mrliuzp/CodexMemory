...
<template>
  ...
  <section class="section-heading"><div><span class="eyebrow">PROJECTS</span><h2>授权项目</h2></div><span class="muted">{{ rows.length }} 个项目</span></section>
  <el-table v-loading="loading" :data="rows" stripe empty-text="暂无项目">
    <el-table-column prop="project_key" label="Project key" min-width="180" /><el-table-column prop="name" label="名称" min-width="180" /><el-table-column prop="repository" label="仓库" min-width="240" /><el-table-column prop="status" label="状态" width="120"><template #default="scope"><el-tag :type="scope.row.status === 'active' ? 'success' : 'info'">{{ scope.row.status }}</el-tag></template></el-table-column>
  </el-table>
</template>
