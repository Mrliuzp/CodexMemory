...
def test_list_api_keys() -> None:
    ...
    resp = client.get("/api/v1/admin/api-keys", headers=_auth("admin-token"))
...
def test_create_api_key() -> None:
    ...
    resp = client.post(
        ...
        json={"project_id": 1, "permissions": ["read"]},
    )
    assert resp.status_code == 200
    data = resp.json()
    assert "token" in data
    assert data["token"].startswith("cm-")
...
def test_delete_api_key() -> None:
    ...
    resp = client.delete(f"/api/v1/admin/api-keys/{key_id}", headers=_auth("admin-token"))
    assert resp.status_code == 200
