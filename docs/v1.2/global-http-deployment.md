...
$NewMcpToken = & .\.venv\Scripts\python.exe -c "import secrets; print(secrets.token_urlsafe(48))"
# 将 .env 中的 CODEX_MEMORY_MCP_TOKEN 替换为 $NewMcpToken；不要输出或提交该值。
...
- 焦点测试结果为 `11 passed`；全量结果为 `217 passed, 1 skipped`，并有 12 条既有弃用警告。
