...
def create_admin_router(session_factory: sessionmaker[Session]) -> APIRouter:
    ...
    def maintenance(payload: dict[str, Any], request: Request, current: Principal = Depends(principal)) -> dict[str, Any]:
        from ..maintenance import MaintenanceService
        if not ({"admin", "operations_write"} & set(current.permissions)):
            raise _error(request, "permission_denied", "缺少维护模式操作权限", status.HTTP_403_FORBIDDEN)
        enabled = bool(payload.get("enabled", False))
        reason = str(payload.get("reason", ""))
        actor = current.project_key or "unknown"
        data = MaintenanceService(session_factory).set_enabled(enabled, reason, actor)
    ...
    def system_status(request: Request, current: Principal = Depends(principal)) -> dict[str, Any]:
        ...
        if not ({"admin", "operations_read"} & set(current.permissions)):
            ...
            raise _error(request, "permission_denied", str(error), status.HTTP_403_FORBIDDEN) from error
        state = OperationsService(session_factory).system_status()
        state["maintenance"] = MaintenanceService(session_factory).current_status()
        return {"data": _redact(state), "meta": {}, "request_id": _request_id(request)}
