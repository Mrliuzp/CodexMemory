...
> [!WARNING]
> 本文档是旧版 `admin/` 原型的历史规划，不再作为正式管理后台的实现依据。当前唯一正式前端是 `apps/admin-web/`，唯一正式管理 API 命名空间是 `/api/admin/v1`；本地 Vite 开发代理应连接正式 API 的 `http://127.0.0.1:8000`。旧版 `admin/static/` 与 `admin/main.py` 仅保留兼容参考，不再继续扩展。
